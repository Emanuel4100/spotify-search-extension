...
Sources are **TypeScript** with **`@girs/*`** typings; **esbuild** emits **`extension.js` / `prefs.js`**. There is **no `tsc` emit** in the default build—run **`npm run typecheck`** for static checking. Plain **JavaScript** would avoid the build step but lose typed GI bindings; the current setup favors maintainability.
